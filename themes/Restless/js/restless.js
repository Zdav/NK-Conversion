$(document).ready(function() {
    ...
    $('#RL_sponsorsPrev').click(function(e){
        e.preventDefault();
        moveSponsors('left');
    })

    $('#RL_sponsorsNext').click(function(e){
        e.preventDefault();
        moveSponsors('right');
    })
...
function moveSponsors(side){
    var maxWidth = parseInt($('#RL_sponsors').css('width'));
    var currentLeft = parseInt($('#RL_sponsors').attr('data-left'));
    var elementWidth = parseInt($('#RL_sponsorsWrapper').attr('data-width'));
    var newValue = null;

    $('.RL_sponsorsCurrent').removeClass('RL_sponsorsCurrent');

    if(side == 'left'){
        if((currentLeft + elementWidth)  > 0){
            newValue = parseInt('-'+(maxWidth-elementWidth));
            $('#RL_sponsors').css('left', newValue);
        }
        else {
            newValue = parseInt(currentLeft + elementWidth);
            $('#RL_sponsors').css('left', newValue);
    ...
    var idElement = (Math.abs(newValue)/elementWidth) + 1;
    $('#RL_sponsorsElement'+idElement).addClass('RL_sponsorsCurrent');

    $('#RL_sponsors').attr('data-left', newValue);
}